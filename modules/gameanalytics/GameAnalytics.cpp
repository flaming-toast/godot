...
#if defined(IOS_PLATFORM)
    /* jeyu: PLEASE NOTE MODIFICATION BELOW!! I had to convert the strings to
     * const char * arrays for this to compile and work properly.
     * Using c_str() doesn't work because the core engine code redefines
     * CharType to wchar_t (wide char) and calling c_str() on the strings gave us a const wchar_t * array (GRRRR)
     */
    char *adSdkName_str = new char[adSdkName.length() + 1];
    memset(adSdkName_str, 0, adSdkName.length() + 1);
    wcstombs(adSdkName_str, adSdkName.c_str(), adSdkName.length() + 1);

    char *adPlacement_str = new char[adPlacement.length() + 1];
    memset(adPlacement_str, 0, adPlacement.length() + 1);
    wcstombs(adPlacement_str, adPlacement.c_str(), adPlacement.length() + 1);

    char *fields_str = new char[fields.length() + 1];
    memset(fields_str, 0, fields.length() + 1);
    wcstombs(fields_str, fields.c_str(), fields.length() + 1);
    if(sendDuration)
    {
        GameAnalyticsCpp::addAdEventWithDuration(adAction, adType, adSdkName_str, adPlacement_str, duration, fields_str, mergeFields);
    }
    else
    {
        GameAnalyticsCpp::addAdEventWithNoAdReason(adAction, adType, adSdkName_str, adPlacement_str, noAdReason, fields_str, mergeFields);
    }
#elif defined(WEB_PLATFORM)
#elif defined(OSX_PLATFORM) || defined(WINDOWS_PLATFORM) || defined(LINUX_PLATFORM)
#endif
